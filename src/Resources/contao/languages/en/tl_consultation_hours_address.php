<?php

declare(strict_types=1);

/*
 * This file is part of Sprechstunden Bundle.
 * 
 * (c) Roland Schale 2022 <roland.schale@gmail.com>
 * @license MIT
 * For the full copyright and license information,
 * please view the LICENSE file that was distributed with this source code.
 * @link https://github.com/rolandschale/consultation-hours-bundle
 */

/**
 * Legends
 */
$GLOBALS['TL_LANG']['tl_consultation_hours_address']['first_legend'] = "Adresse";
$GLOBALS['TL_LANG']['tl_consultation_hours_address']['second_legend'] = "Kontakt";

/**
 * Operations
 */
$GLOBALS['TL_LANG']['tl_consultation_hours_address']['edit'] = ["Datensatz mit ID: %s bearbeiten", "Datensatz mit ID: %s bearbeiten"];
$GLOBALS['TL_LANG']['tl_consultation_hours_address']['copy'] = ["Datensatz mit ID: %s kopieren", "Datensatz mit ID: %s kopieren"];
$GLOBALS['TL_LANG']['tl_consultation_hours_address']['delete'] = ["Datensatz mit ID: %s löschen", "Datensatz mit ID: %s löschen"];
$GLOBALS['TL_LANG']['tl_consultation_hours_address']['show'] = ["Datensatz mit ID: %s ansehen", "Datensatz mit ID: %s ansehen"];

/**
 * Fields
 */
$GLOBALS['TL_LANG']['tl_consultation_hours_address']['name_1'] = ["Name 1", "Geben Sie einen Namen ein"];
$GLOBALS['TL_LANG']['tl_consultation_hours_address']['name_2'] = ["Name 2", "Geben Sie einen Namen ein"];
$GLOBALS['TL_LANG']['tl_consultation_hours_address']['name_3'] = ["Name 3", "Geben Sie einen Namen ein"];
$GLOBALS['TL_LANG']['tl_consultation_hours_address']['street'] = ["Straße", "Geben Sie eine Straße mit Hausnummer ein"];
$GLOBALS['TL_LANG']['tl_consultation_hours_address']['zip'] = ["PLZ", "Geben Sie die PLZ ein"];
$GLOBALS['TL_LANG']['tl_consultation_hours_address']['city'] = ["Ort", "Geben Sie den Ortsnamen ein"];

$GLOBALS['TL_LANG']['tl_consultation_hours_address']['phone'] = ["Telefon", "Geben Sie eine Telefonnummer ein"];
// $GLOBALS['TL_LANG']['tl_consultation_hours_address'][''] = ["Chosen Feld", "Wählen Sie aus."];
// $GLOBALS['TL_LANG']['tl_consultation_hours_address'][''] = ["Multitext Feld", "Geben Sie die Werte ein"];
// $GLOBALS['TL_LANG']['tl_consultation_hours_address']['addSubpalette'] = ["Erweiterte Einstellungen aktivieren", "Hier können Sie die erweiterten Einstellungen aktivieren."];
// $GLOBALS['TL_LANG']['tl_consultation_hours_address'][''] = ["", "Geben Sie einen Text ein"];

/**
 * References
 */
// $GLOBALS['TL_LANG']['tl_consultation_hours_address']['firstoption'] = "Erste Option";
// $GLOBALS['TL_LANG']['tl_consultation_hours_address']['secondoption'] = "Zweite Option";

/**
 * Buttons
 */
$GLOBALS['TL_LANG']['tl_consultation_hours_address']['customButton'] = "Custom Routine starten";
